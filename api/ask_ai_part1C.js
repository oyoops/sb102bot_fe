...
module.exports = async (req, res) => {
    ...
    */
    ...
    const messages = [{
        ...
        "content": `
            CONTEXT:
                You are reviewing a parcel that is INELIGIBLE for any Live Local Act benefits.
                However, you shall describe what the benefits WOULD be if the user was able to find a qualifying, eligible property nearby.

                MAX DENSITY:
                    The parcel is ${subject_isInCity} city limits, so its primary municipality is ${displayMuniName}.
                    Using the Live Local Act, eligible properties (of which this is NOT one) are allowed to match ${displayMuniName}'s highest-allowed residential density.
                        - ${displayMuniName}'s max density is ${maxMuniDensity} units/acre.

                MAX HEIGHT:
                    Although this parcel is not qualified, the Live Local Act allows for buildings on *qualifying* properties to be built as high as the tallest building within a one mile radius.
                        - The tallest such building around here is:
